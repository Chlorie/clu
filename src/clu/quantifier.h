...
namespace clu
{
    ...
    public:
        ...
        template <typename U> friend constexpr bool operator==(const elements& lhs, const U& rhs)
        {
            return lhs.check(rhs, std::equal_to<>{});
        }
        template <typename U> friend constexpr bool operator==(const U& lhs, const elements& rhs)
        {
            return rhs == lhs;
        }
        template <typename U> friend constexpr bool operator!=(const elements& lhs, const U& rhs)
    ...
    auto all_of(Ts&&... values) { return elements<quantifier_type::all, Ts&&...>{ std::forward<Ts>(values)... }; }
    template <typename... Ts>
    auto any_of(Ts&&... values) { return elements<quantifier_type::any, Ts&&...>{ std::forward<Ts>(values)... }; }
    template <typename... Ts>
    auto none_of(Ts&&... values) { return elements<quantifier_type::none, Ts&&...>{ std::forward<Ts>(values)... }; }
}
