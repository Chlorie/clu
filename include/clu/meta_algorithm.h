...
namespace clu::meta
{
    ...
    struct contains
    {
        ...
        static constexpr bool apply_v = (std::is_same_v<Target, Types> || ...);
        template <typename... Types>
        using apply = std::bool_constant<apply_v<Types...>>;
    };
    ...
    template <typename...> struct unique : std::type_identity<type_list<>> {};
    ...
    struct set_include<Set1, type_list<Types2...>> :
        std::bool_constant<(Set1::template contains<Types2>::value && ...)> {};
