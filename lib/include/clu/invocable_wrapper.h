...
namespace clu
{
    template <typename Inv> class invocable_wrapper;

    template <typename Inv>
        requires std::is_class_v<Inv> && (!std::is_final_v<Inv>)
    ...
    public:
        ...
        constexpr explicit invocable_wrapper(Inv2&& inv)
            noexcept(std::is_nothrow_constructible_v<Inv, Inv2&&>):
