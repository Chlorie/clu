...
namespace clu
{
    ...
    constexpr auto make_piper(Inv&& invocable)
        noexcept(std::is_nothrow_constructible_v<std::remove_cvref_t<Inv>, Inv&&>);
    // @formatter:on
    ...
    class piper_mixin
    {
        ...
        constexpr friend decltype(auto) operator|(Arg&& first, Self&& self)
            noexcept(std::is_nothrow_invocable_v<Self, Arg&&>)
        {
            return static_cast<Self&&>(self)
                (static_cast<Arg&&>(first));
        }
        // @formatter:on
            ...
            std::is_nothrow_constructible_v<std::remove_cvref_t<Self>, Self&&>)
        {
            ...
            return clu::make_piper(compound{
...
#define CLU_COMPOUND_PIPER_CALL_IMPL(cnst, ref)                                                            \
            template <typename Arg> requires                                                               \
                std::invocable<cnst First ref, Arg&&> &&                                                   \
                std::invocable<cnst Second ref, std::invoke_result_t<cnst First ref, Arg&&>>               \
            constexpr decltype(auto) operator()(Arg&& arg) cnst ref noexcept(                              \
                std::is_nothrow_invocable_v<cnst First ref, Arg&&> &&                                      \
                std::is_nothrow_invocable_v<cnst Second ref, std::invoke_result_t<cnst First ref, Arg&&>>) \
            {                                                                                              \
                return static_cast<cnst First ref>(first)(                                                 \
                    static_cast<cnst Second ref>(second)(                                                  \
                        static_cast<Arg&&>(arg)));                                                         \
            }                                                                                              \
            static_assert(true)
    ...
    namespace detail
    {
        ...
        public:
            ...
            constexpr explicit bind_back_t(Inv2&& inv, Args2&&... args) noexcept(
                ...
                invocable_(static_cast<Inv2&&>(inv)), args_(static_cast<Args2&&>(args)...) {}
...
#define CLU_BIND_BACK_CALL_IMPL(cnst, ref)                                         \
            ...
            {                                                                      \
                ...
                {                                                                  \
                    return std::invoke(static_cast<cnst Inv ref>(invocable_),      \
...
#undef CLU_BIND_BACK_CALL_IMPL
    ...
    constexpr auto bind_back(Inv&& invocable, Args&&... args) noexcept(
        std::is_nothrow_constructible_v<Inv, Inv&&> &&
        (std::is_nothrow_constructible_v<Args, Args&&> && ...))
    {
