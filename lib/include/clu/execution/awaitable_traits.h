...
namespace clu::exec
{
    ...
    inline struct get_awaiter_t
    {
    private:
        template <typename A>
        constexpr static auto get_awaiter_impl(A&& a, priority_tag<2>)
        CLU_SINGLE_RETURN(static_cast<A&&>(a).operator co_await());

        template <typename A>
        constexpr static auto get_awaiter_impl(A&& a, priority_tag<1>)
        CLU_SINGLE_RETURN(operator co_await(static_cast<A&&>(a)));

        template <typename A>
        constexpr static decltype(auto) get_awaiter_impl(A&& a, priority_tag<0>) noexcept { return static_cast<A&&>(a); }
