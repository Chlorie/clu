...
namespace clu
{
    ...
    namespace detail
    {
        ...
        template <size_t byte_size, std::endian E> requires (byte_size % 4 == 0)
        ...
        public:
            constexpr void update(const_buffer bytes) noexcept
            {
                ...
                length_ += bytes.copy_to_consume(
                    mutable_buffer(block_) += static_cast<size_t>(length_ % block_size));
    ...
    private:
        ...
        constexpr void process_block(const block32_t& block) noexcept
        {
            // SHA1 operations
            const auto r0 = [&](const size_t i,
                uint32_t& a, const uint32_t b, const uint32_t c, const uint32_t d)
            {
        ...
        constexpr const auto& get_digest() noexcept
        {
            ...
            for (uint32_t& v : digest_) v = detail::switch_byte_order_u32(v);
            return digest_;
        }
