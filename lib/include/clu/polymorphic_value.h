...
namespace clu
{
    ...
    namespace detail
    {
        ...
        public:
            ...
            {
                return std::make_unique<inplace_control_block>(*this);
            }
        ...
        public:
            ...
            std::unique_ptr<control_block<T>> clone() const override
            {
                return std::make_unique<pointer_control_block>(copier_(*ptr_), copier_, ptr_.get_deleter());
            }
        ...
        public:
            ...
            std::unique_ptr<control_block<T>> clone() const override
            {
                return std::make_unique<delegating_control_block>(cb_->clone());
            }
    ...
    public:
        ...
        explicit polymorphic_value(U* ptr, C copier = C{}, D deleter = D{})
        {
            ...
            cb_ =
                std::make_unique<detail::pointer_control_block<T, U, C, D>>(ptr, std::move(copier), std::move(deleter));
        }
    ...
    {
        return polymorphic_value<T>(std::in_place, std::forward<Ts>(args)...);
    }

    template <typename T, typename U, typename... Ts>
    [[nodiscard]] auto make_polymorphic_value(Ts&&... args)
    {
        return polymorphic_value<T>(std::in_place_type<U>, std::forward<Ts>(args)...);
    }
} // namespace clu
