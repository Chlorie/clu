...
namespace clu
{
    ...
    public:
        ...
        {
            if (init.size() > N)
                throw std::out_of_range("initializer list too large for this static_vector");
            clear();
        ...
        [[nodiscard]] auto operator<=>(const static_vector& other) const
        {
            return std::ranges::lexicographical_compare(*this, other);
        }
