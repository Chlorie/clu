...
namespace clu
{
    ...
    [[nodiscard]] constexpr auto to_underlying(const Enum value) noexcept { return static_cast<std::underlying_type_t<Enum>>(value); }
    ...
    public:
        ...
        constexpr explicit(false) flags(const Enum bit) noexcept:
            data_(static_cast<data_type>(1) << to_underlying(bit)) {}
        ...
        [[nodiscard]] constexpr friend flags operator&(const flags lhs, const flags rhs) noexcept { return flags(lhs.data_ & rhs.data_); }
        [[nodiscard]] constexpr friend flags operator|(const flags lhs, const flags rhs) noexcept { return flags(lhs.data_ | rhs.data_); }
        [[nodiscard]] constexpr friend flags operator^(const flags lhs, const flags rhs) noexcept { return flags(lhs.data_ ^ rhs.data_); }
        ...
        [[nodiscard]] constexpr bool is_set(const flags bits) const noexcept { return (data_ & bits.data_) == bits.data_; }
        [[nodiscard]] constexpr bool is_unset(const flags bits) const noexcept { return (~data_ & bits.data_) == bits.data_; }
    ...
    inline namespace flag_enum_operators
    {
        ...
        [[nodiscard]] constexpr flags<Enum> operator&(const Enum lhs, const Enum rhs) noexcept { return flags<Enum>(lhs) & rhs; }
        template <flag_enum Enum>
        [[nodiscard]] constexpr flags<Enum> operator|(const Enum lhs, const Enum rhs) noexcept { return flags<Enum>(lhs) | rhs; }
        template <flag_enum Enum>
        [[nodiscard]] constexpr flags<Enum> operator^(const Enum lhs, const Enum rhs) noexcept { return flags<Enum>(lhs) ^ rhs; }
        template <flag_enum Enum>
        [[nodiscard]] constexpr flags<Enum> operator~(const Enum bit) noexcept { return ~(flags<Enum>(bit)); }
    }
}
